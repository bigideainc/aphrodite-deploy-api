...
async def setup_deployment_files(ssh_client):
...
async def setup_container(ssh_client, model_id: str, user_id: str, 
    ...
    await setup_deployment_files(ssh_client)
...
async def monitor_container_startup(ssh_client, container_id: str, timeout: int = 300):
    logger.info(f"Monitoring container {container_id} startup...")
    ...
    while not startup_complete and (datetime.now() - start_time).total_seconds() < timeout:
        if stdout.channel.recv_ready():
            ...
            if "http://localhost:2242/" in line:
                endpoints["base_url"] = "http://localhost:2242/"
